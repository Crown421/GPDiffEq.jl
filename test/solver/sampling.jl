...
@testset "1D" begin
    ...
    s1itp = GPDiffEq.PullSolversModule._create_itps(
        xrange1, s1, BSpline(Quadratic(Line(OnGrid()))), Line()
    )
...
s2itp = GPDiffEq.PullSolversModule._create_itps(
    xrange2, s2, BSpline(Quadratic(Line(OnGrid()))), Line()
)
