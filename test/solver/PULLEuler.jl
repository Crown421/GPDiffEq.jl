...
function old_solve(prob, alg::PULLEuler; dt, kwargs...)
    ...
    xe = zeros(Measurement{Float64}, length(ts))
    av = zeros(length(ts))
    ...
    for (i, tstep) in enumerate(tsteps)
        old_linearized_eulerstep!(gp, dgp, xe, av, tstep, i; lhist=alg.buffersize)
    end
...
@testset "Scalar" begin
    ...
    refsol = old_solve(prob, PULLEuler(); dt=h)
...
@testset "2D" begin
    ...
    @test getfield.(sol.u, :μ) ≈ refdetsol.u atol = 1e-6

    # ToDo: Add variance and convergence testing
